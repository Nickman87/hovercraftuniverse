...
#include <windows.h>
#include "Ogre.h"
// Ogre 14 API fix (docs/porting/ogre-api-gap.md): Ogre::DisplayString now
// lives in the Overlay component, see FlashValue.h.
...
public:
	/**
	* HU team patch (reconstructed, see docs/porting/hikari-gui.md): sets
	* whether this FlashControl participates in mouse hit-testing/focus at
	* all. Used to let purely-informational HUD overlays (e.g. a
	* speedometer) sit on top without stealing clicks meant for whatever is
	* behind them.
	*
	* @param	val		Whether this FlashControl should handle mouse input.
	*/
	void handleInputs(bool val);

	/**
	* Returns whether this FlashControl currently handles mouse input (see handleInputs).
	*/
	bool isHandlingInputs() { return handlingInputs; }
