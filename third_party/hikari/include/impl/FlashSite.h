...
// upstream imported the Flash ActiveX control's typelib via its registered
// PROGID ("PROGID:ShockwaveFlash.ShockwaveFlash"), which requires
// Flash.ocx to be registered (regsvr32) on the build machine. This modern
// tree never registers ActiveX controls system-wide, so instead #import
// the typelib directly out of the Flash.ocx file (a "computed #import",
// same macro-expansion mechanism as a computed #include) via
// HIKARI_FLASH_OCX_PATH, which hu_hikari (modern/CMakeLists.txt) defines
// to the absolute, quoted path of the Flash.ocx this tree builds against.
