...
// Modern-build fix (not a team patch -- see impl/FlashSite.h and
// docs/porting/hikari-gui.md for the full rationale): import the typelib
// directly from the Flash.ocx file instead of via a registered PROGID.
#ifndef HIKARI_FLASH_OCX_PATH
#error "HIKARI_FLASH_OCX_PATH must be defined (as a quoted path) to the Flash.ocx to import the typelib from -- see hu_hikari in modern/CMakeLists.txt"
#endif
#import HIKARI_FLASH_OCX_PATH named_guids
...
// Ogre::DisplayString (as returned by FlashValue::getString) is now narrow
// (Ogre::String), but this file's XML-ish invocation protocol is wide
// throughout (it round-trips through BSTR/VARIANT to talk to the Flash
// ActiveX control). Widen ASCII-range only, same scope as FlashValue.cpp's
// narrowFromWide.
inline std::wstring widenToWide(const std::string& narrow)
{
...
std::wstring serializeValue(const FlashValue& value)
{
	...
	case FT_STRING:
		{
			std::wstring stringVal = widenToWide(value.getString());
