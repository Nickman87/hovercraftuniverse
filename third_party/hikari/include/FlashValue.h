...
#include "Ogre.h"
// Ogre 14 API fix (docs/porting/ogre-api-gap.md): Ogre::DisplayString moved
// out of core Ogre and into the Overlay component (Ogre::DisplayString is
// now declared in OgreOverlayElement.h) when Overlay was split out as a
// separate component.
...
public:
	...
	*/
	...
	// Ogre 14 API fix (docs/porting/ogre-api-gap.md /
	// docs/porting/hikari-gui.md): Ogre::DisplayString is now just a
	// typedef for Ogre::String (narrow) -- modern Ogre's Overlay component
	// dropped the old wide-string "unicode display" support entirely. That
	// collapses what used to be a distinct FlashValue(const
	// Ogre::DisplayString&) overload onto the FlashValue(const
	// std::string&) overload below (duplicate-declaration error), so that
	// overload was removed; the wchar_t*/std::wstring overloads that
	// remain now narrow (ASCII-range only, sufficient for
	// ActionScript-side identifiers/text this library actually exchanges)
	// instead of relying on Ogre's old string class's implicit wide/narrow
	// conversions.
