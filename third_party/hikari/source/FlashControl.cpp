...
FlashControl::~FlashControl()
{
	...
	// Ogre 14 API fix (docs/porting/ogre-api-gap.md): Ogre::SharedPtr no
	// longer has setNull()/isNull()/getPointer() (it's now a thin wrapper
	// around std::shared_ptr) -- setNull() becomes reset().
	texture.reset();
...
{
	// Ogre 14 API fix (docs/porting/ogre-api-gap.md / docs/porting/hikari-gui.md):
	// funcName is narrow (Ogre::DisplayString == Ogre::String now); widen
	// it since Impl::serializeInvocation's XML-ish invocation protocol is
	// wide throughout (see impl/flashhandler.h).
	BSTR returnVal = 0;
	HRESULT result = flashInterface->raw_CallFunction(_bstr_t(Impl::serializeInvocation(Impl::widenToWide(funcName), args).c_str()), &returnVal);
...
void FlashControl::handleInputs(bool val)
{
	handlingInputs = val;
}
...
bool FlashControl::isPointOverMe(int screenX, int screenY)
{
	...
	// HU team patch (reconstructed, see docs/porting/hikari-gui.md): a
	// FlashControl with input handling turned off (see handleInputs) never
	// participates in mouse hit-testing/focus, so purely-informational HUD
	// overlays don't steal clicks meant for whatever is behind them.
	if(!handlingInputs)
		return false;
...
void FlashControl::handleFlashCall(const std::wstring& xmlString)
{
	...
	// Ogre 14 API fix (docs/porting/ogre-api-gap.md / docs/porting/hikari-gui.md):
	// delegateMap is keyed by Ogre::DisplayString (narrow, == Ogre::String
	// now), but funcName came back from the wide invocation protocol
	// (impl/flashhandler.h); narrow it (ASCII-range, same as
	// FlashValue.cpp's narrowFromWide) before looking it up.
	Ogre::String narrowFuncName(funcName.begin(), funcName.end());
	DelegateIter i = delegateMap.find(narrowFuncName);
	if(i != delegateMap.end())
	{
...
void FlashControl::update()
{
	...
	// HU team patch (reconstructed, see docs/porting/hikari-gui.md): the
	// mainContext/mainBitmap DC cache used to be keyed off
	// lastDirtyWidth/lastDirtyHeight members and only rebuilt when the dirty
	// region's size changed between frames. Those two members were dropped;
	// the DC/bitmap pair is now unconditionally rebuilt whenever the dirty
	// region's size differs from what mainBitmap was last sized for (tracked
	// via GetObject instead of a cached member), which is simpler and avoids
	// a class of stale-cache bugs at a small, one-time-per-resize cost.
	BITMAP currentBitmapInfo = {0};
	if(mainBitmap)
		GetObject(mainBitmap, sizeof(BITMAP), &currentBitmapInfo);

	if(!mainContext || currentBitmapInfo.bmWidth != dirtyWidth || currentBitmapInfo.bmHeight != dirtyHeight)
	{
		if(mainContext)
		{
