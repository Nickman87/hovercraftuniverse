...
: viewport(viewport), width(width), height(height), position(pos), isVisible(true), zOrder(zOrder)
{
...
void ViewportOverlay::setZOrder(Ogre::ushort zOrder)
{
	this->zOrder = zOrder;
	// Ogre::Overlay::setZOrder only accepts [0, 650] (RENDER_QUEUE_OVERLAY-1);
	// clamp defensively since Hikari's zOrder is no longer capped to [0, 99].
	overlay->setZOrder(zOrder > 650 ? 650 : zOrder);
}

Ogre::ushort ViewportOverlay::getZOrder()
{
	return zOrder;
}
