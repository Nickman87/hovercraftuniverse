...
namespace {
	...
	// narrow a wide string down to Ogre::DisplayString (== Ogre::String,
	// narrow) now that Ogre's Overlay component no longer has a
	// wide-string-capable display-string type. ASCII-range only, which
	// covers everything this library actually exchanges with ActionScript
	// (function names, numbers, "true"/"false").
...
FlashValue::FlashValue(const wchar_t* stringValue) : strValue(narrowFromWide(stringValue)), numValue(0), boolValue(0), valueType(FT_STRING)
{
}
...
FlashValue::FlashValue(const std::wstring& stringValue) : strValue(narrowFromWide(stringValue.c_str())), numValue(0), boolValue(0), valueType(FT_STRING)
{
}
