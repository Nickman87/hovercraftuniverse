...
HikariManager::HikariManager(const std::string& assetsDirectory) : flashLib(0), focusedControl(0), mouseXPos(0), mouseYPos(0),
	mouseButtonRDown(false), zOrderCounter(1), keyboardHook(0), mBringToTop(true)
{
	if(instance)
...
FlashControl* HikariManager::createFlashOverlay(const Ogre::String& name, Ogre::Viewport* viewport, int width, int height, const Position& position, Ogre::ushort zOrder)
{
	...
	if(zOrder == 0)
	{
		...
		if(highestZOrder == -1)
			...
			zOrder = (Ogre::ushort)(highestZOrder + 1);
	}
...
void HikariManager::toggleBringToTop(bool val)
{
	mBringToTop = val;
}
...
bool HikariManager::focusControl(int x, int y, FlashControl* selection)
{
	...
	// original multi-tier z-reordering ("bring the focused control to the
	// front of its tier") is now gated behind mBringToTop/toggleBringToTop,
	// and no longer partitions by tier (tiers were removed). When disabled,
	// focusing a control still grants it keyboard/mouse focus but leaves
	// the render order (z-order) untouched.
	if(mBringToTop)
	{
		...
		if(sortedControls.size())
		{
			if(sortedControls.at(0) != controlToFocus)
			{
				...
				if(popIdx < sortedControls.size())
				{
					Ogre::ushort highestZ = sortedControls.at(0)->overlay->getZOrder();
